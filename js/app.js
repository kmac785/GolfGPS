...
let currentHeading = null;        // latest compass heading (degrees)
let hasCompassSensor = false;     // true if device orientation events fire
let compassPermissionAsked = false;
let headingSmoothed = null;       // smoothed heading for less jitter
...
map.on('rotate', () => {
    const bearing = map.getBearing();
    // Show north button when rotated AND heading mode is off
    // (when heading mode is on, the compass button replaces it)
    if (Math.abs(bearing) > 2 && !isHeadingActive) {
...
// ============================================================
// Compass heading — auto-rotation
// ============================================================
const headingBtn = document.getElementById('headingBtn');

function smoothHeading(raw) {
    // Exponential smoothing to reduce compass jitter
    if (headingSmoothed === null) {
        headingSmoothed = raw;
        return raw;
    }
    // Handle wrap-around (e.g. 359 → 1)
    let diff = raw - headingSmoothed;
    if (diff > 180) diff -= 360;
    if (diff < -180) diff += 360;
    headingSmoothed += diff * 0.3; // smoothing factor (0 = no update, 1 = instant)
    // Normalize to 0–360
...
function applyHeading(rawDeg) {
    ...
    currentHeading = rawDeg;
    const smoothed = smoothHeading(rawDeg);
    map.rotateTo(-smoothed, { duration: 200, easing: (t) => t });
}
...
function listenOrientation() {
    ...
    window.addEventListener('deviceorientationabsolute', (e) => {
        if (e.alpha != null) {
            ...
            // On Android, absolute alpha = degrees from true north (clockwise)
            // But alpha is actually *counter*-clockwise in the spec,
            // so heading = 360 - alpha
            applyHeading(360 - e.alpha);
        }
    });

    // iOS: uses webkitCompassHeading (degrees from magnetic north, clockwise)
...
function toggleHeading() {
    ...
    if (isHeadingActive) {
        // Request compass permission on first activation (must be from user tap)
        if (!compassPermissionAsked) {
            requestCompassPermission();
        }
        headingBtn.classList.add('active');
        northBtn.classList.remove('visible'); // hide north btn while heading is on
        // Also re-enable follow mode
        isFollowing = true;
        if (playerLocation) {
            map.easeTo({ center: [playerLocation.lng, playerLocation.lat], duration: 500 });
        }
    } else {
...
// Disable heading mode when user manually rotates or pans
map.on('touchstart', (e) => {
    // Two-finger touch = manual rotation → disable heading
...
function startGPS() {
    ...
    navigator.geolocation.watchPosition(
        (pos) => {
            ...
            // GPS heading fallback: use when moving and no compass sensor
            if (!hasCompassSensor && isHeadingActive &&
                pos.coords.heading != null && pos.coords.speed > 0.5) {
                applyHeading(pos.coords.heading);
            }
...
map.on('dragstart', () => {
    isFollowing = false;
    // Disable heading when user pans
    if (isHeadingActive) {
        isHeadingActive = false;
        headingBtn.classList.remove('active');
        headingSmoothed = null;
